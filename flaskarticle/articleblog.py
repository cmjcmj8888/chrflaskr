...
from flaskr.articleauth import login_required
from flaskr.articledb import get_db


bp = Blueprint('articleblog', __name__)
...
def index():
    ...
    return render_template('articleblog/index.html', posts=posts)
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO tb_article (title, body, author_id)'
                ' VALUES (?, ?, ?)',
    ...
    return render_template('articleblog/create.html')
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE tb_article SET title = ?, body = ?'
                ' WHERE id = ?',
                (title, body, id)
            )
            db.commit()
    ...
    return render_template('articleblog/update.html', post=post)
...
def delete(id):
    get_post(id)
    db = get_db()
    db.execute('DELETE FROM tb_article WHERE id = ?', (id,))
    db.commit()
    return redirect(url_for('articleblog.index'))

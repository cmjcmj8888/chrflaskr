...
bp = Blueprint( 'articlemark', __name__, url_prefix= '/articlemark')
...
def index():
    ...
    marks = db.execute(
        ...
        ' FROM mark p JOIN user u ON p.author_id = u.id'
        ' ORDER BY created DESC'
    ).fetchall()
    return render_template('articlemark/index.html', marks=marks)
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO mark (title, body, author_id)'
                ' VALUES (?, ?, ?)',
                (title, body, g.user['id'])
            )
            db.commit()
            return redirect(url_for('articlemark.index'))

    return render_template('articlemark/create.html')
...
def get_mark(id, check_author=True):
    ...
    if check_author and mark['author_id'] != g.user['id']:
        abort(403)
...
def update(id):
    ...
    return render_template('articlemark/update.html', mark=mark)

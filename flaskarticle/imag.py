...
bp = Blueprint('imag', __name__, url_prefix='/imag')
...
def index():
    ...
    imags = db.execute(
        ...
        ' FROM imag p JOIN user u ON p.author_id = u.id'
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO imag (title, body, author_id)'
                ' VALUES (?, ?, ?)',
...
def get_imag(id, check_author=True):
    ...
    if check_author and imag['author_id'] != g.user['id']:
        abort(403)
